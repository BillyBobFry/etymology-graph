...
const expandButtonLabel = computed(() => (props.expanded ? "Collapse graph" : "Expand graph"));
...
<template>
  <div :class="graphControlsClass" role="group" aria-label="Graph controls">
    <IconButton label="Zoom out" size="sm" @click="emitControlAction('zoom-out')">
      <Minus :size="16" stroke-width="2.75" aria-hidden="true" />
    </IconButton>
    <output class="grid min-h-[34px] min-w-[54px] place-items-center font-label text-[13px] font-bold text-text-muted" aria-live="polite">{{ zoomPercentage }}%</output>
    <IconButton label="Zoom in" size="sm" @click="emitControlAction('zoom-in')">
      <Plus :size="16" stroke-width="2.75" aria-hidden="true" />
    </IconButton>
    <IconButton label="Reset graph layout" size="sm" @click="emitControlAction('reset')">
    ...
    <GraphCanvasGuidePopover v-model:open="isGraphGuideOpen" />
  </div>
</template>
