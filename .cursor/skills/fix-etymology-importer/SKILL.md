...
description: Investigate and fix incorrect etymology graph edges from Wiktextract seed data. Use when the user flags a bad graph, ancestry API result, doublet result, seed expansion issue, etymology_templates issue, or importer bug for a specific term such as en:wait, en:bat, en:channel, or en:character.
---
...
Turn a reported bad graph into a focused importer regression and a narrow fix. Prefer real Wiktextract seed evidence, seed-expansion behavior, merged-neighborhood traversal, graph preview output, and TDD over broad refactors or guessing from the API response alone.
...
5. If the target's ancestry depends on flat `etymology_templates`, descendant lists, or entries not present on the target page, inspect and model the merged neighborhood: seed-enqueued ancestor targets, the ancestor/source entries, their structured descendants/derived records, and traversal from the original target.
6. Compare these things before editing:
   ...
   - seed target expansion: which records should be enqueued from `etymology_templates`, descendants, and derived lists
   - merged preview/traversal output across the relevant records, not only the target entry
...
7. State the suspected bug in graph terms, for example `en:king` should not fan out through every Middle English spelling variant, `fro:trois -> la:trēs` is missing because a rendered etymon tree was ignored, or `en:channel` is missing ancestry because seed expansion never processes the ancestor pages whose descendant lists provide the intermediate edges.
8. Add a focused failing test in `packages/importer/src/wiktextract.test.ts` before changing implementation. Prefer:
   - a small inline `WiktextractEntry` only when the issue is local to one entry conversion
   - a trimmed real fixture group when the bug depends on seed expansion plus multiple entries
   - a merged-neighborhood traversal assertion with `previewStructuredEntry()` outputs and `traverseAncestors()` when the API graph is wrong
9. Run `pnpm --filter @etymology-graph/importer test -- wiktextract.test.ts` and confirm the new test fails for the expected seed, edge, or traversal difference.
10. Make the smallest importer or seed-extractor change that fixes the pattern.
11. Re-run `pnpm --filter @etymology-graph/importer test -- wiktextract.test.ts`, then check lints on edited files.

## Interpretation Rules

- Treat flat `etymology_templates` as seed-expansion hints, not graph edges. `args["1"]` is the current entry language; source language and term are usually `args["2"]` and `args["3"]`. Enqueue supported source targets broadly, then let imported structured records provide real edges.
...
- For ancestry bugs like `en:channel`, do not stop at the target entry preview. Assert the pipeline: target entry enqueues source terms, source records import descendant/derived edges such as `la:canalis -> fro:canel`, and merged traversal reaches the expected ancestors.

## Common Patterns

- Missing seed ancestor: inspect the structured ancestry frontier and add/adjust template-hint extraction in `extract-structured-ancestry-seed.ts`; regression-test the target queue as well as final edges.
- Missing DB edge with records present: inspect `previewStructuredEntry()` for etymon-tree, descendants, derived, affix, or compound extraction gaps. If the API needs edges from multiple declaring entries, test the merged neighborhood.
